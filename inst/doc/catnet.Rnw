...
In conclusion, \Rpackage{catnet} does not intend to replace the existing, in many cases much more efficient, learning algorithms but to provide non-heuristic, Maximum Likelihood based, set of tools for statistical inference. 
...
The \Rpackage{catnet} package for \Robject{R} provides an inference framework for discrete Bayesian networks with the intention to diversify the existing tools for network statistical modeling. Implemented strictly according to the Maximum Likelihood  principle, it targets the more statistical oriented community. It provides MLE solutions for networks with known order of their nodes and is able to perform stochastic search in the space of orders. The inference functions of the package can handle data with missing values and data with perturbations, suitable for micro-array analysis as well as, soft and hard edge constraints on the graph structure. 
The additional functions for sampling, prediction and network comparison complete the \Rpackage{catnet} package to a self-contained framework for discrete network analysis. 
