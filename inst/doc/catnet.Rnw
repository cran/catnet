...
\Rpackage{catnet} package implements \underline{cat}egorical Bayesian \underline{net}work framework in \Robject{R}.
...
the conditional distributions are multinomial with no additional parametric constrains. 
...
Two main techniques are implemented - finding the best network fitting some data for a predefined node order and stochastic search of optimal networks without constrains on the node order. 
...
Although the authors of \Rpackage{catnet} are having in mind reconstruction of gene/protein regulatory networks as a primary application of the package (for overview  on this topic see \cite{paola} and \cite{friedman00}), its functionality is by no means limited for use to bioinformatics only 
...
Let $\mathcal{A}=\{x_i\}_{i=1}^n$ be a set of $n$ discrete random variables 
with number of categories $\mathcal{C}=\{c_i\}_{i=1}^n$. 
A categorical network $\mathcal{G}$ with nodes $\mathcal{A}$ is described by 
its parenthood structure and conditional probabilities. 
We denote by $\Pi_i$ the parent set of the $i$-th node, thus, 
$\Pi_i\subset \mathcal{A}$ and every $x_j\in\Pi_i$ is a parent of $x_i$. The conditional probability of $x_i$ given $\Pi_i$ is denoted $P(x_i|\Pi_i)$ and follows unconstrained categorical distribution.
Any permutation $\Omega$ of size $n$ defines an order of the nodes $\mathcal{A}$. We say that $\Omega$ is consistent with $\mathcal{G}$ if 
...
Let $\mathcal{X}=\{X^{j}\}_{j=1}^{k}$ be a sample of $k$ observations on variables $\mathcal{A}$. 
...
A categorical network can be created explicitly by calling \Rfunction{cnNew} function. The function takes following arguments: a vector of node names (\Rfunction{nodes}), 
...
The complexity of a \Robject{catNetwork} object can be obtained by calling \Rfunction{cnComplexity} function. 
...
\Rpackage{catnet} searches for networks fitting the data exhaustively, according to the MLE criterion. 
When the topological order of the nodes is known, 
the package provides a function, \Robject{cnSearchOrder}, that finds the exact 
MLE solution of the estimation problem. Otherwise, the user may choose one of the 
stochastic search functions \Robject{cnSearchSA} or \Robject{cnSearchSAcluster}. 
...
$$
where the likelihood $\mathcal{L}(\mathcal{X} | \mathcal{G})$ is defined by equation \ref{eq:likelihood}. 
...
the user may assign mandatory parents to some nodes. These two parameters allow a variety of edge  constrains to be implemented. 
...
These functions implement a Simulated Annealing (SA) algorithm. More technical details are given below.

For a sample $\mathcal{X}$ of observations on variables $\mathcal{A}$ and an order $\Omega$, we have defined $\hat\mathcal{G}(\Omega, T)$ to be the network with complexity $T$ consistent with $\Omega$ 
...
Moreover, we assume that $\Omega$ is a random element of the space of all permutations of nodes $\mathcal{A}$ and its probability is 
$$
...
By varying \Robject{orderShuffles}, the user can control the extend of $\mathcal{S}(\Omega)$.

The complexity $T$ used in equation \ref{eq:cn_mc_accept}, also called target complexity, may vary from one iteration of SA to another. It is determined by a user specified selection criterion, the parameter \Robject{selectMode}. Note that the SA search is only optimal for the series of networks, one for each SA iteration, having complexities $T$. 
